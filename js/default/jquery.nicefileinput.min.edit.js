...
{
	// инициализация
	$("input[type=file]").nicefileinput({ label: "Обзор..." });

	// стилизуем кнопки
	$('.NFI-button').addClass('action-button');

	// фиксим баг: после обновления страницы файл остаётся выбранным, но не отображается
	$('input.NFI-current').each(function(index, element)
	{
		$this = $(element);
		$this.parents('.NFI-wrapper').find('.NFI-filename').val($this.val());
	});
});